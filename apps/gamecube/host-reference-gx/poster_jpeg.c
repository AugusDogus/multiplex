  ...
  for (unsigned y = 0; y < MULTIPLEX_GATEWAY_ARTWORK_HEIGHT; ++y) {
    for (unsigned x = 0; x < MULTIPLEX_GATEWAY_ARTWORK_WIDTH; ++x) {
      ...
      const uint8_t chroma_u =
          picture->data[1][(source_y >> chroma_v_shift) * picture->linesize[1] +
                           (source_x >> chroma_h_shift)];
      const uint8_t chroma_v =
          picture->data[2][(source_y >> chroma_v_shift) * picture->linesize[2] +
                           (source_x >> chroma_h_shift)];
      const uint16_t pixel =
          yuv_to_rgb565(luma, chroma_u, chroma_v, full_range);
      const size_t tile =
          ((size_t)(y / 4u) * (MULTIPLEX_GATEWAY_ARTWORK_WIDTH / 4u) + x / 4u) *
          32u;
...
static bool decode_jpeg(const uint8_t *encoded, size_t encoded_size,
  ...
  if (encoded == NULL || encoded_size == 0 || item_count == 0 ||
      ...
      columns > ATLAS_COLUMNS || texture_pixels == NULL ||
      texture_capacity < required || encoded_size > INT32_MAX) {
    return false;
  }
  ...
  const unsigned expected_width = columns * MULTIPLEX_GATEWAY_ARTWORK_WIDTH;
  const unsigned expected_rows = (item_count + columns - 1u) / columns;
  const unsigned expected_height =
      expected_rows * MULTIPLEX_GATEWAY_ARTWORK_HEIGHT;
  const bool dimensions_match =
      allow_center_crop ? context->width >= (int)expected_width &&
  ...
  const bool planar_yuv = full_range || context->pix_fmt == PIX_FMT_YUV420P ||
  ...
  if (consumed < 0 || got_picture == 0 || !dimensions_match || !planar_yuv ||
      ...
      picture->data[2] == NULL) {
    SYS_Report("REFERENCE GX: invalid poster JPEG consumed=%d got=%d "
               "size=%dx%d format=%d expected=%ux%u\n",
  ...
  const unsigned crop_x =
      allow_center_crop ? ((unsigned)context->width - expected_width) / 2u : 0u;
  const unsigned crop_y =
      allow_center_crop ? ((unsigned)context->height - expected_height) / 2u
                        : 0u;
  ...
  SYS_Report(
      ...
      item_count, context->width, context->height, context->pix_fmt, consumed);
