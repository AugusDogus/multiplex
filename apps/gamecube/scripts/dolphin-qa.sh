...
require_dolphin() {
  if ! dolphin_pid >/dev/null; then
    ...
    echo "Launch it with: bun run gamecube:qa -- launch" >&2
    exit 1
  fi
}
...
launch() {
  ...
  systemd-run --user --collect \
    ...
    --setenv="GAMECUBE_PASTA_BIN=${GAMECUBE_PASTA_BIN:-$app_dir/.passt/pasta}" \
...
check() {
  ...
  assert_latency_budget "Search results" 500000 "$search_values"
  stable_fps=$(
