...
export function ServerLibraryGroup({ server, state, sources, onRetry }: ServerLibraryGroupProps) {
  ...
  if (state.isLoading && !state.isRetrying) {
    return (
      <SidebarGroup>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <div className="text-muted-foreground px-2 py-1 text-xs">Loading libraries...</div>
          </SidebarMenuItem>
        </SidebarMenu>
      </SidebarGroup>
    );
  ...
  if (state.error || state.data?.error || state.data === null) {
    return (
      <SidebarGroup>
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <div className="text-muted-foreground px-2 pb-1 text-xs">No libraries available</div>
          </SidebarMenuItem>
        </SidebarMenu>
      </SidebarGroup>
    );
        ...
        {sources.length === 0 ? (
          <SidebarMenuItem>
            <SidebarMenuButton disabled>
              <span className="text-muted-foreground text-sm">No libraries found</span>
            </SidebarMenuButton>
          </SidebarMenuItem>
        ) : (
          sources.map((source) => {
