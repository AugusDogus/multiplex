...
>()("@multiplex/desktop/app/DesktopShutdown") {}
...
const make = Effect.gen(function* () {
  ...
  return DesktopShutdown.of({
    ...
    markComplete: Ref.getAndSet(didComplete, true).pipe(
      Effect.flatMap((alreadyCompleted) =>
        alreadyCompleted ? Effect.void : Deferred.succeed(completed, undefined).pipe(Effect.asVoid),
      ),
    ),
