...
// The hosted-service adapter still consumes this state and dispatches these
// messages directly. They are intentionally outside the declarative view.
export const viewUnbound = [
  ...
  "searchLoaded",
  "selectedFromSearch",
  "watchTogetherInviteesLoaded",
  "watchTogetherInviteesAvailable",
  "watchTogetherInviteePage",
  "selectedWatchTogetherInviteeId",
  "watchTogetherLoaded",
  "watchTogetherAvailable",
  "watchTogetherCreating",
  "selectedWatchTogetherRoomIndex",
  "watchTogetherLeaveRequested",
  "watchTogetherReconnectRequested",
  "watchTogetherDisbandRequested",
  "detailsChildrenStart",
  "detailsChildrenTotal",
  "detailsChildrenLoaded",
  "playbackOffsetMs",
  "playing",
  "subtitleStreamCount",
  "selectedSubtitleStream",
  "markWatchedRequested",
  "playbackNavigationRequest",
  "libraryActionsAvailable",
  "playbackSubtitleSelection",
  "markWatchedRequestRatingKey",
  "playbackRequestedNavigation",
  "catalogPreviewTitle",
  "catalogPreviewSecondary",
  "catalogPreviewHierarchy",
  "catalogPreviewHasHierarchy",
  "catalogPreviewHasProgress",
  "catalogPreviewRemainingMinutes",
  "homeCarouselPreviousDisabled",
  "homeCarouselNextDisabled",
  "homeCarouselSelectionAtStart",
  "homeCarouselSelectionAtEnd",
  "rowCount",
  "hasMultipleRows",
  "hasResume",
  "detailsRequestRatingKey",
  "playbackRequestRatingKey",
  "playbackRequestOffsetMs",
  "detailsUnplayable",
  "detailsChildrenHasPrevious",
  "detailsChildrenHasNext",
  "detailsChildrenRequestRatingKey",
  "detailsChildrenRequestStart",
  "hasLibraries",
  "browseSelectionAtTop",
  "browseSelectionAtBottom",
  "browseHasPrevious",
  "browseHasNext",
  "browseRequestSection",
  "browseRequestStart",
  "searchPrompt",
  "searchBeforeCursor",
  "searchAfterCursor",
  "searchHasQuery",
  "watchTogetherCreateRatingKey",
  "watchTogetherCreateTitle",
  "watchTogetherCreateInviteeId",
  "selectedWatchTogetherRoomTitle",
  "watchTogetherInactive",
  "watchTogetherDisconnected",
  "watchTogetherJoinRequestIndex",
  "searchRequestQuery",
  "rows",
  "browseItems",
  ...
  "detailsHistory",
  "subtitleStreams",
  "homeCarouselPeekItems",
  "homeNextShelfPeekItems",
  "homeFollowingShelfPeekItems",
  "browseContinuationItems",
  "keyboardRowOne",
  "keyboardRowTwo",
  "keyboardRowThree",
  "browse_previous_row",
...
const demoItems: readonly CatalogItem[] = [
  ...
  },
  {
    id: 4,
    ratingKey: 5,
    title: asciiBytes("Severance"),
    subtitle: asciiBytes("S2 E3"),
    secondary: asciiBytes("Who Is Alive?"),
    hierarchy: asciiBytes("S2 E3"),
    hasHierarchy: true,
    imageId: 5,
    durationMs: 3_420_000,
    viewOffsetMs: 1_163_000,
    progressPercent: 34,
  },
  {
    id: 5,
    ratingKey: 6,
    title: asciiBytes("The Bear"),
    subtitle: asciiBytes("S3 E4"),
    secondary: asciiBytes("Violet"),
    hierarchy: asciiBytes("S3 E4"),
    hasHierarchy: true,
    imageId: 6,
    durationMs: 2_010_000,
    viewOffsetMs: 1_387_000,
    progressPercent: 69,
  },
  {
    id: 6,
    ratingKey: 7,
    title: asciiBytes("Arrival"),
    subtitle: asciiBytes("2016"),
    secondary: asciiBytes("2016"),
    hierarchy: new Uint8Array(0),
    hasHierarchy: false,
    imageId: 7,
  ...
  },
  {
    id: 7,
    ratingKey: 8,
    title: asciiBytes("Blade Runner 2049"),
    subtitle: asciiBytes("2017"),
    secondary: asciiBytes("2017"),
    hierarchy: new Uint8Array(0),
    hasHierarchy: false,
    imageId: 8,
    durationMs: 9_840_000,
    viewOffsetMs: 0,
    progressPercent: 0,
  },
];
...
const demoRows: readonly CatalogRow[] = [
  ...
  { id: 2, title: asciiBytes("Recently Added Movies"), items: demoItems },
  { id: 3, title: asciiBytes("Recently Added TV"), items: demoItems },
];
...
export function initialModel(): Model {
  return {
    ...
    detailsSecondary: asciiBytes("Featured"),
    detailsType: asciiBytes("Movie"),
    detailsLibrary: asciiBytes("Movies"),
    detailsContentRating: new Uint8Array(0),
    detailsFacts: new Uint8Array(0),
    detailsSummary: asciiBytes("Ready to watch from your Multiplex library."),
...
// Paired builds need a connected catalog source. The standalone desktop demo
// uses its bundled catalog so every navigation path remains interactive.
export function libraryActionsAvailable(model: Model): boolean {
  return model.pairingEnabled ? model.gatewayConnected && model.pairingLinked : true;
}
...
export function previewCatalogItem(model: Model, index: number): Model {
  const items = catalogItems(model);
  if (!(index >= 0 && index <= 1_000_000) || index >= items.length) return model;
  const item = items[index];
  return {
    ...model,
    selectedIndex: Math.trunc(index),
...
function updateItemProgress(item: CatalogItem, positionMs: number): CatalogItem {
  const safePositionMs =
    positionMs >= 0 && positionMs <= 9_000_000_000_000 ? Math.trunc(positionMs) : 0;
  const lastPositionMs =
    item.durationMs > 0 && item.durationMs <= 9_000_000_000_000 ? item.durationMs - 1 : 0;
  const viewOffsetMs = safePositionMs < lastPositionMs ? safePositionMs : lastPositionMs;
  const rawProgressPercent =
    item.durationMs <= 0 ? 0 : Math.min(100, intDiv(viewOffsetMs * 100, item.durationMs));
  const progressPercent =
    rawProgressPercent >= 0 && rawProgressPercent <= 100 ? Math.trunc(rawProgressPercent) : 0;
...
function commitSelectedProgressAt(model: Model, positionMs: number): Model {
  const safePositionMs =
    positionMs >= 0 && positionMs <= 9_000_000_000_000 ? Math.trunc(positionMs) : 0;
  const lastPositionMs =
    model.selectedDurationMs > 0 && model.selectedDurationMs <= 9_000_000_000_000
      ? model.selectedDurationMs - 1
      : 0;
  const selectedViewOffsetMs = safePositionMs < lastPositionMs ? safePositionMs : lastPositionMs;
...
export function update(model: Model, msg: Msg): Model {
  switch (msg.kind) {
    ...
    case "open_home":
      return model.screen === "player"
        ? model
        : { ...model, screen: "home", startMenuOpen: false, playerSettingsOpen: false };
    case "previous_row": {
      if (model.rowIndex <= 0 || model.rowIndex > 1_000_000) return model;
      ...
      return previewCatalogItem(
        {
          ...model,
          rowIndex: Math.trunc(rowIndex),
          rowNumber: Math.trunc(rowIndex + 1),
          homeCarouselStart: 0,
        },
        selectedIndex,
      );
    ...
    case "next_row": {
      if (
        model.rowIndex < 0 ||
        model.rowIndex >= 1_000_000 ||
        model.rowIndex + 1 >= model.rows.length
      )
        return model;
      const rowIndex = model.rowIndex >= 0 && model.rowIndex < 1_000_000 ? model.rowIndex + 1 : 0;
      const selectedIndex = Math.min(
        model.rows[rowIndex].items.length - 1,
        Math.max(0, model.selectedIndex - model.homeCarouselStart),
      );
    ...
    case "browse_previous_row": {
      if (
        model.screen !== "browse" ||
        model.browseStart <= 0 ||
        model.browseStart > 1_000_000 ||
        model.selectedIndex < 0 ||
        model.selectedIndex > 1_000_000
      )
        return model;
      const start =
        model.browseStart > browseColumns && model.browseStart <= 1_000_000
          ? model.browseStart - browseColumns
          : 0;
      const selectedColumn =
        model.selectedIndex >= 0 && model.selectedIndex <= 1_000_000
          ? model.selectedIndex % browseColumns
          : 0;
      return {
        ...model,
        browsePendingStart:
          start >= 0 && start <= 1_000_000 ? Math.trunc(start) : model.browseStart,
        selectedIndex:
          selectedColumn >= 0 && selectedColumn <= 1_000_000 && selectedColumn < browseColumns
            ? Math.trunc(selectedColumn)
            : 0,
        browseFailed: false,
      };
    }
    case "browse_next_row": {
      if (
        ...
        !browseHasNext(model) ||
        model.browseStart < 0 ||
        model.browseStart > 999_000 ||
        model.selectedIndex < 0 ||
        model.selectedIndex > 1_000_000
      )
        return model;
      const nextBrowseStart = model.browseStart + browseColumns;
      const selectedColumn =
        model.selectedIndex >= 0 && model.selectedIndex <= 1_000_000
          ? model.selectedIndex % browseColumns
          : 0;
      const nextSelectedIndex =
        selectedColumn >= 0 && selectedColumn <= 1_000_000 && selectedColumn < browseColumns
          ? browseColumns + selectedColumn
          : 0;
      return {
        ...model,
        browsePendingStart:
          nextBrowseStart >= 0 && nextBrowseStart <= 1_000_000
            ? Math.trunc(nextBrowseStart)
            : model.browseStart,
        selectedIndex:
          nextSelectedIndex >= 0 && nextSelectedIndex <= 1_000_000
            ? Math.trunc(nextSelectedIndex)
            : 0,
        browseFailed: false,
      };
    }
    ...
    case "search_edit": {
      if (model.screen !== "search") return model;
      const next = applyTextInputEvent(
        {
          text: model.searchQuery,
          selection: { anchor: model.searchCursor, focus: model.searchCursor },
          composition: null,
        },
        msg.edit,
        64,
      ...
      return {
        ...model,
        searchQuery: next.text,
        searchCursor: next.selection.focus,
      };
    }
    case "search_key": {
    ...
    case "search_delete":
      if (model.screen !== "search" || model.searchCursor <= 0 || model.searchCursor > 24)
        return model;
      return {
        ...model,
        searchQuery: deleteSearchKey(model.searchQuery, model.searchCursor),
        searchCursor:
          model.searchCursor > 0 && model.searchCursor <= 24 ? model.searchCursor - 1 : 0,
      };
    ...
    case "search_cursor_right":
      if (
        ...
        model.searchCursor >= 24 ||
        model.searchCursor >= model.searchQuery.length
      )
        return model;
      return {
        ...model,
        searchCursor:
          model.searchCursor >= 0 && model.searchCursor < 24 ? model.searchCursor + 1 : 24,
      };
    case "search_submit":
      if (model.screen !== "search" || model.searchQuery.length === 0) return model;
      if (!model.pairingEnabled) {
        const results = demoItems.filter((item) =>
          containsIgnoreCase(item.title, model.searchQuery),
        );
        return {
          ...model,
    ...
    case "watch_together_invitees_next":
      if (
        ...
        model.watchTogetherInviteePage < 0 ||
        model.watchTogetherInviteePage >= 1_000_000 ||
      ...
      return {
        ...model,
        watchTogetherInviteePage:
          model.watchTogetherInviteePage >= 0 && model.watchTogetherInviteePage < 1_000_000
            ? model.watchTogetherInviteePage + 1
            : 0,
      };
    ...
    case "open_details_child": {
      ...
      const historyLength = model.detailsHistory.length;
      if (!(historyLength >= 0 && historyLength <= 1_000_000)) return model;
      const parent: CatalogItem = {
        id: Math.trunc(historyLength),
    ...
    case "sync_playback": {
      if (
        ...
        !model.playbackLoaded ||
        model.selectedDurationMs <= 1 ||
        model.selectedDurationMs > 9_000_000_000_000
      ) {
        return model;
      }
      const safePositionMs =
        msg.positionMs >= 0 && msg.positionMs <= 9_000_000_000_000 ? Math.trunc(msg.positionMs) : 0;
      const lastPositionMs =
        model.selectedDurationMs > 1 && model.selectedDurationMs <= 9_000_000_000_000
          ? model.selectedDurationMs - 1
          : 0;
      const playbackOffsetMs = safePositionMs < lastPositionMs ? safePositionMs : lastPositionMs;
      return { ...model, playbackOffsetMs: Math.trunc(playbackOffsetMs) };
    }
