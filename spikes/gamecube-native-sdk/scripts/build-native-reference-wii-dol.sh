...
if [ ! -s "$spike_dir/.libogc2-stage/opt/devkitpro/libogc2/wii/lib/libogc.a" ]; then
  echo "Missing the pinned libogc2 Wii runtime; run bun run spike:gamecube:bootstrap first." >&2
  exit 1
fi
...
mkdir -p "$spike_dir/build-native-reference-wii"
sh "$script_dir/generate-media-source-header.sh" \
  "$spike_dir/build-native-reference-wii/media-source.h"
sh "$script_dir/generate-tls-ca-header.sh" \
  "$spike_dir/build-native-reference-wii/tls-ca.h"
...
test -s "$spike_dir/multiplex-wii-native-reference.dol"
readelf -sW "$spike_dir/multiplex-wii-native-reference.elf" |
  grep -q "multiplex_native_app_render_reference"

dol_size=$(wc -c <"$spike_dir/multiplex-wii-native-reference.dol")
echo "Wii Native reference DOL is ready at $spike_dir/multiplex-wii-native-reference.dol ($dol_size bytes)"
