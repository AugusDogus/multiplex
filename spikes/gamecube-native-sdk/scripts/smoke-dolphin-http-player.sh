...
python3 "$script_dir/serve-http-fixture.py" "$port" \
  "$spike_dir/assets/multiplex-dvd-demo.mpg" \
  >"$spike_dir/.http-fixture.log" 2>&1 &
server_pid=$!
...
while ! curl --noproxy '*' --fail --silent --output /dev/null "$local_fixture_url"; do
  if ! kill -0 "$server_pid" 2>/dev/null; then
    ...
    sed -n '1,80p' "$spike_dir/.http-fixture.log" >&2 || true
    exit 1
  fi
  attempt=$((attempt + 1))
...
if ! cmp -s "$downloaded_fixture" \
  "$spike_dir/assets/multiplex-dvd-demo.mpg"; then
...
if [ "$rootless_tap" -eq 1 ]; then
  ...
  dolphin_config_profile="$spike_dir/dolphin/Dolphin.bba.ini"
