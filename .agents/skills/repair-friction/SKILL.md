---
name: repair-friction
description: Repair a reproducible defect in Multiplex-owned code discovered while running a supported Multiplex workflow. Use after a minimized case reproduces on the current default branch. Do not use when the only failure is invalid input with an actionable diagnostic, documented unsupported behavior, an unresolved product decision, or a fix owned by another repository.
---

# Repair Multiplex friction

Take one confirmed Multiplex defect from a minimized reproduction to a reviewed
...
1. Create an isolated worktree and a dedicated branch from the refreshed default
   branch. Leave the discovering agent's checkout and changes untouched.
