...
2. **Confirm it is an effect-atom mutation boundary.** The setter should come from `@effect/atom-react` and a mutation atom from a local `atoms` module (e.g. watch-together session atoms under `apps/web`).
...
5. **Do not rewrite unrelated local async code.** Probe requests, OAuth popup helpers, `fetch`, and browser APIs may need a different skill unless the finding specifically points at the mutation call.
...
- Keep existing typed error handling when present.
...
- `try/catch` around non-effect APIs such as `new URL`, `JSON.parse`, raw `fetch`, or browser popup code. Those may be real findings, but they need a different remediation skill.
- `useAtomSet(..., { mode: "promise" })` with no local failure handling. Some call sites intentionally let callers decide the boundary.
- Tests or server Effect code. This skill is for React/effect-atom UI mutation handlers.
- Manual optimistic placeholder cleanup. Use `effect-atom-optimistic` for that; if both patterns appear together, fix optimistic plumbing first, then use `promiseExit` for the remaining mutation boundary.
